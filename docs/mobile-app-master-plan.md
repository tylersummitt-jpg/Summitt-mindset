...
*Project-control document. Version 1.5.16. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.15 history retained in prior revisions. **v1.5.16 (2026-07-21) — APP-015 physical native link/navigation audit PASS:** legal/support links proven in-app; native purchase suppression physically proven; Safari Free Trial/Subscribe preserved; Film Room Vimeo playback/navigation PASS; no mobile-repo change required. **v1.5.15:** public in-app deletion E2E PASS. **v1.5.14:** code-side discoverability. **v1.5.13:** Create account + membership-gate PASS. **v1.5.12:** APP-021/022 + controlled backend E2E.*
...
| Current phase | **Store / portal path** — auth, membership gate, public in-app deletion, and **APP-015 native link/navigation** all physically PASS (2026-07-21). Apple Developer enrollment waiting on **D-U-N-S**. Icon/splash open (Brooke). Store metadata / privacy answers next. TestFlight open. Android deferred. |
| Current assigned task IDs | APP-042 / store metadata + privacy answers, APP-049+ (Apple Developer when D-U-N-S available / App Store Connect), APP-046–048 (icon/splash/screenshots when assets ready), APP-040, APP-065, APP-066 |
| Last completed task IDs | **APP-015 physical native link/navigation PASS (2026-07-21)**; website native-safe link fixes; **Public in-app deletion E2E PASS**; Create account + CAPTCHA + membership-gate PASS; APP-041 controlled E2E PASS; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash; store metadata / privacy answers / TestFlight. Account deletion and APP-015 link audit no longer store-submission blockers. APP-065 parallel. Android unstarted. |
...
| Exact next task | **1)** Prepare App Store and Play Store metadata / privacy answers (APP-042 / related). **2)** Complete Apple organization enrollment when **D-U-N-S** arrives (APP-049+). **3)** Add final icon and launch assets when Brooke finishes (APP-046–047). **4)** Create App Store Connect record and configure signing. **5)** Archive and upload TestFlight (APP-050). **6)** Android implementation and Play Console afterward. Parallel: **APP-065**, **APP-066**. Do **not** add IAP. Do **not** add Sign in with Apple for V1. |
...
1. **Store metadata / privacy answers** — App Store and Play Store listing answers, review credentials (APP-042 / related).
2. **Apple Developer organization enrollment** — when D-U-N-S is available (APP-049+).
3. **App icon + launch/splash assets** — when Brooke supplies final files (APP-046–047); do **not** mark complete until then.
...
> **APP-015 COMPLETE** — physical native link/navigation PASS recorded 2026-07-21 (SESSION 45).
...
- **v1.5.16 dependency notes:** **APP-021 COMPLETE** (iOS-first). **APP-022** iOS identity COMPLETE. Native auth + membership gate **physically PASS**. **APP-041 public in-app deletion COMPLETE**. **APP-015 COMPLETE / physical PASS**. Icon/splash open. Apple portal waiting on D-U-N-S. Store metadata next. TestFlight open. **APP-065 IN PROGRESS**. Android APP-018+ deferred.
...
- **Parallelizable:** APP-065; APP-066; APP-042 drafting; icons/splash; Android only after iOS stable.
...
### CRITICAL PATH
**iPhone Stage 1 POC PASS (A2)** → **Clerk 180-day config COMPLETE** → **APP-010/016/069 COMPLETE** → **APP-021 COMPLETE (iOS-first A2)** → **APP-022 iOS identity COMPLETE** → **APP-041 public in-app deletion COMPLETE (2026-07-21)** → **app Sign in + Create account + membership gate PASS** → **APP-015 physical native link/navigation PASS (2026-07-21)** → store metadata / privacy answers → **Apple Developer when D-U-N-S available** → icon/splash when Brooke finishes → App Store Connect / TestFlight / submit → **Android later** when iOS stable. Parallel: APP-065, APP-066.

### FIRST GO/NO-GO TEST
**Does the Clerk session persist in an iOS WKWebView after force-close/reopen and land the user in Victory Room without re-login?** (Checkpoint A / APP-011.) **Stage 1: PASS** (including full reboot). Production max lifetime **180 days** (DEC-022); short-cycle post-change reopen PASS. Identity/entitlement formally verified. **APP-015 COMPLETE.** Remaining: **APP-065 elapsed-time proof**, Client Trust, App Store posture (metadata / D-U-N-S / assets / TestFlight), Android.
...
**APP-015 physical native link/navigation PASS (2026-07-21).** Deletion + auth + membership gate + reviewer links COMPLETE. Exact next: **1)** Prepare App Store and Play Store metadata / privacy answers. **2)** Complete Apple organization enrollment when **D-U-N-S** arrives. **3)** Add final icon and launch assets when Brooke finishes. **4)** Create App Store Connect record and configure signing. **5)** Archive and upload TestFlight. **6)** Android implementation and Play Console afterward. Parallel: APP-065, APP-066. Do **not** rebuild the deletion backend. Do **not** reopen APP-015 without new evidence. Do **not** claim Apple enrollment / TestFlight / App Review / Android complete.

---

*End of master plan v1.5.16. Maintained per §14. Do not let it become aspirational fiction — update statuses and evidence every session.*
