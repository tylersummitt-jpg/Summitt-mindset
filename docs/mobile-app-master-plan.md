...
| Exact next task | **1)** Verify deletion discoverability in the native shell before any public activation. **2)** Implement and verify app-specific email-only sign-in/access behavior (DEC-018 / APP-061 preferred mechanism). **3)** iOS production hardening / privacy / assets (APP-040, APP-046–048, shell polish). **4)** Apple Developer / App Store Connect / TestFlight path (APP-049+). **5)** Android later when iOS is sufficiently stable. Parallel: **APP-065**, **APP-015**, **APP-066**. Store-facing privacy/`/data-deletion` copy is in place (APP-041F IN PROGRESS). Do **not** rebuild the deletion backend. Do **not** enable public deletion without explicit approval. |
