...
| Current blocker | **APP-065** elapsed-time proof still open. **APP-041** parent still open for native-shell deletion discoverability verification and **public** initiation activation (store-facing privacy/`/data-deletion` copy in place; public path intentionally off). **Website `/app/sign-in` email-code surface implemented** — mobile shell start URL not yet pointed at it. Android unstarted. |
