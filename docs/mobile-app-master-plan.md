...
*Project-control document. Version 1.5.19. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.18 history retained in prior revisions. **v1.5.19 (2026-07-21) — native Meta Pixel suppression COMPLETE / physical PASS:** Safari Web Inspector on physical iPhone — zero facebook/fbevents/connect.facebook rows; `typeof window.fbq === "undefined"` on Victory Room and in-app `/`; website Pixel unchanged. **v1.5.18:** code-side native Pixel suppression. **v1.5.17:** store-submission package. **v1.5.16:** APP-015 physical PASS. **v1.5.15:** public in-app deletion E2E PASS.*
...
| Current phase | **Store / portal path** — auth, membership gate, deletion, APP-015 PASS; store drafts READY; **native Meta Pixel suppression COMPLETE / physical PASS**. Apple enrollment waiting on **D-U-N-S**. Icon/splash open (Brooke). TestFlight open. Android deferred. |
| Current assigned task IDs | APP-049+ (Apple Developer when D-U-N-S available / App Store Connect), APP-046–048 (icon/splash/screenshots when assets ready), APP-042 portal finalize (Tyler copy / website Meta disclosure / PrivacyInfo), APP-040, APP-065, APP-066 |
| Last completed task IDs | **Native Meta Pixel suppression COMPLETE / physical PASS (2026-07-21)**; website native Pixel gate; **APP-042 store-submission drafts**; **APP-015 physical PASS**; public in-app deletion E2E PASS; Create account + membership-gate PASS; APP-041; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash; Tyler store-copy / website Meta disclosure; PrivacyInfo review; TestFlight. Native Meta Pixel no longer a store-submission blocker. APP-065 parallel. Android unstarted. |
...
| Exact next task | **1)** Tyler store-copy decisions + website privacy Meta disclosure (Pixel remains on Safari). **2)** Complete Apple organization enrollment when **D-U-N-S** arrives (APP-049+). **3)** Icon/splash when Brooke finishes; screenshots. **4)** App Store Connect + TestFlight. **5)** Android later. Parallel: **APP-065**, **APP-066**. Do **not** add IAP. Do **not** claim “no data collected.” |
...
1. **Tyler store decisions + remaining privacy items** — subtitle/category; website privacy-policy Meta/Vimeo/Resend naming; PrivacyInfo completeness (`docs/store-submission/`). Native Meta Pixel PASS — not open.
...
**Native Meta Pixel suppression COMPLETE / physical PASS (2026-07-21).** Exact next: **1)** Tyler store-copy decisions + website Meta disclosure in privacy policy. **2)** Complete Apple organization enrollment when **D-U-N-S** arrives. **3)** Add final icon and launch assets when Brooke finishes; capture screenshots. **4)** Create App Store Connect record and configure signing. **5)** Archive and upload TestFlight. **6)** Android implementation and Play Console afterward. Parallel: APP-065, APP-066. Do **not** invent “no data collected.” Do **not** claim Apple/Google approval. Do **not** reopen native Pixel without new evidence.

---

*End of master plan v1.5.19. Maintained per §14. Do not let it become aspirational fiction — update statuses and evidence every session.*
