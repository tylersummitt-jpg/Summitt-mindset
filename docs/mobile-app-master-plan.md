...
*Project-control document. Version 1.5.18. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.17 history retained in prior revisions. **v1.5.18 (2026-07-21) — native Meta Pixel suppression:** RootLayout omits `MetaPixelRoot` for `SummittMindsetiOS` UA; website Pixel unchanged; physical iPhone verification still required. **v1.5.17:** store-submission package. **v1.5.16:** APP-015 physical PASS. **v1.5.15:** public in-app deletion E2E PASS.*
...
| Current phase | **Store / portal path** — auth, membership gate, deletion, APP-015 PASS; store drafts READY; **native Meta Pixel suppressed in code** (physical proof open). Apple enrollment waiting on **D-U-N-S**. Icon/splash open (Brooke). TestFlight open. Android deferred. |
| Current assigned task IDs | APP-049+ (Apple Developer when D-U-N-S available / App Store Connect), APP-046–048 (icon/splash/screenshots when assets ready), APP-042 portal finalize (Tyler decisions + physical Pixel check + PrivacyInfo), APP-040, APP-065, APP-066 |
| Last completed task IDs | **Native Meta Pixel suppression (website code, 2026-07-21)**; **APP-042 store-submission draft package**; **APP-015 physical PASS**; public in-app deletion E2E PASS; Create account + membership-gate PASS; APP-041; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash; Tyler store-copy decisions; **physical native Meta Pixel verification**; PrivacyInfo review; TestFlight. APP-065 parallel. Android unstarted. |
...
| Exact next task | **1)** Physical iPhone verify: native WebView has zero Meta Pixel (`connect.facebook.net` / `fbevents.js` / `fbq`). **2)** Tyler store-copy / privacy-policy Meta disclosure for website. **3)** Complete Apple organization enrollment when **D-U-N-S** arrives (APP-049+). **4)** Icon/splash when Brooke finishes; screenshots. **5)** App Store Connect + TestFlight. **6)** Android later. Parallel: **APP-065**, **APP-066**. Do **not** add IAP. Do **not** claim “no data collected.” |
...
| Analytics | Meta Pixel (marketing routes; **omitted for native iOS UA**) | `src/components/MetaPixelRoot.tsx`, `src/lib/meta-pixel*.ts`, `src/app/layout.tsx` | Yes (browser) | Add app-level analytics/crash (native) | Low |
