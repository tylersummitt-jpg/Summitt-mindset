...
*Project-control document. Version 1.5.13. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.12 history retained in prior revisions. **v1.5.13 (2026-07-21) — documentation reconciliation:** physical iPhone **Create account + membership-gate PASS** (email-code, Clerk CAPTCHA, inactive → `/app/membership`, no pricing/Checkout/IAP); mobile repo `522a3a9294bbf080dc6e070ff6fdfbf0cd382185` records PASS + Debug-only Safari Web Inspector (Release non-inspectable). **v1.5.12 (2026-07-20):** APP-021 COMPLETE (iOS-first A2); APP-022 iOS identity COMPLETE; APP-041F4b COMPLETE; APP-041 controlled E2E PASS; public deletion still intentionally disabled; DEC-019 ACTIVE; remaining APP-041 work is store-facing privacy/discoverability/activation.*
...
| Current phase | **Store / portal path** — native app auth + membership gate **physically proven** (2026-07-21). **APP-041** still IN PROGRESS for native-shell deletion discoverability + **public** initiation activation (backend already E2E PASS). **APP-021 COMPLETE**. **APP-022** iOS identity COMPLETE. **APP-065** IN PROGRESS. Apple Developer / App Store Connect / TestFlight **not started**. Icon/splash **paused** awaiting Brooke. Android deferred. |
| Current assigned task IDs | APP-049+ (Apple Developer / App Store Connect), APP-046–048 (icon/splash/screenshots when assets ready), APP-041 (deletion discoverability + public activation later), APP-015, APP-040, APP-042, APP-065, APP-066 |
| Last completed task IDs | Physical `/app/sign-in` Sign in + Create account + CAPTCHA + inactive membership-gate PASS (2026-07-21); native purchase suppression proven on new-account path; mobile Debug-only Safari inspector at `522a3a9…`; APP-041 controlled E2E PASS (2026-07-20), APP-041F4b–A, APP-022, APP-021, APP-010/016/069, APP-062–064, APP-008 Stage 1, APP-059, APP-070 |
| Current blocker | **No auth/membership-gate blocker.** Remaining: Apple Developer Program / App Store Connect identity; app icon + launch assets (Brooke); external-link + account-deletion discoverability; store metadata / TestFlight; public deletion activation before submission. **APP-065** elapsed-time proof still open (parallel). Android unstarted. |
...
| Account deletion | **Backend built + production-proven** (controlled disposable-account E2E PASS 2026-07-20). **Public account deletion intentionally disabled** (`ACCOUNT_DELETION_INITIATION_ENABLED` absent; temporary test flags removed after the proof). Remaining: store-facing privacy/deletion documentation, discoverability, and later explicit public activation **before App Store submission**. |
| Exact next task | **1)** Apple Developer Program / App Store Connect identity setup (APP-049+). **2)** App icon + launch/splash assets when Brooke finishes (APP-046–047; do **not** mark complete until final files land). **3)** Remaining external-link + account-deletion discoverability checks (APP-015 / APP-041). **4)** App Store metadata, privacy answers, screenshots, review account. **5)** Archive + TestFlight (APP-050). **6)** Final public account-deletion activation before submission. **7)** Android after iOS path stabilizes. Parallel: **APP-065**, **APP-066**. Do **not** add IAP. Do **not** enable public deletion without explicit approval. Do **not** add Sign in with Apple for V1. |
...
1. **Apple Developer Program / App Store Connect identity** — APP-049+.
2. **App icon + launch/splash assets** — when Brooke supplies final files (APP-046–047); do **not** mark complete until then.
3. **External-link + account-deletion discoverability** — APP-015 / APP-041 (before public deletion activation).
4. **App Store metadata** — privacy answers, screenshots, review account (APP-048 / APP-053 prep).
5. **Archive + TestFlight** — APP-050.
6. **Final public account-deletion activation** — only with explicit approval, before App Review submission.
7. **APP-065 / APP-066** — elapsed-time session proof; Client Trust formal confirmation (parallel).
