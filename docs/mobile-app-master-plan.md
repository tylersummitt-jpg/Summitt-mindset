...
*Project-control document. Version 1.5.23. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.22 history retained in prior revisions. **v1.5.23 (2026-07-22) — canonical native platform detection:** website `detectSummittMindsetPlatform` / `isNativeSummittMindsetApp` supports `none` | `ios` | `android` (`SummittMindsetiOS` / `SummittMindsetAndroid`); product gates (Meta Pixel, checkout, CTAs, membership/sign-in routing, deletion redirect) use native-app detection; Android native shell **not** created; existing iOS behavior preserved; physical iPhone regression after deploy still required; Android device validation future. **v1.5.22:** physical Vimeo `dnt=1` playback PASS. **v1.5.21:** Vimeo embed privacy posture. **v1.5.20:** Meta Privacy Policy disclosure. **v1.5.19:** native Meta Pixel physical PASS.*
...
| Current phase | **Store / portal path** — auth, membership gate, deletion, APP-015 PASS; native Meta Pixel physical PASS; Vimeo `dnt=1` PASS; **canonical native platform detection (`none`/`ios`/`android`) COMPLETE on website** (Android shell not built); store drafts READY. Apple enrollment waiting on **D-U-N-S**. Icon/splash open (Brooke). TestFlight open. Android deferred. |
...
| Last completed task IDs | **Canonical native platform detection (2026-07-22)**; **Physical Vimeo `dnt=1` playback PASS**; **Vimeo embed hardening + Privacy Policy Vimeo disclosure**; **Public Privacy Policy Meta disclosure**; **Native Meta Pixel suppression COMPLETE / physical PASS**; **APP-042 store-submission drafts**; **APP-015 physical PASS**; public in-app deletion E2E PASS; Create account + membership-gate PASS; APP-041; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash; Tyler store listing copy; PrivacyInfo review; TestFlight; Google Play org enrollment; **Android native shell (not started)**; physical iPhone regression after this website deploy. APP-065 parallel. |
...
*End of master plan v1.5.23. Maintained per §14. Do not let it become aspirational fiction — update statuses and evidence every session.*
