...
| Exact next task | **Review APP-041F2** (unreachable `POST /api/account/delete`). Then commit/deploy unreachable → unauthorized + disabled production smoke → F3 Danger Zone UI behind initiation flag → controlled F4 before public activation. Do **not** claim deletion is live. Do **not** enable flags. Parallel: **APP-065**, **APP-015**, **APP-066**, **APP-021**. |
...
- **APP-041F2:** **IMPLEMENTED — UNREACHABLE — PENDING REVIEW** — `POST /api/account/delete` dual-flag gated; server-derived identity/key; exact `DELETE`; Clerk reverification fail-closed; no UI; both flags off; no real deletion request created.
- Exact next action: **review F2** → commit/deploy unreachable → unauthorized + disabled production smoke → F3 Danger Zone UI behind initiation flag → controlled F4 before public activation. Canonical matrix: `docs/account-deletion-purge-matrix.md`.
...
| APP-041F2 | Unreachable initiation route foundation | **IMPLEMENTED — UNREACHABLE — PENDING REVIEW** | ~2–3 |
...
### Exact next action
**Review APP-041F2** (unreachable `POST /api/account/delete`). Then commit/deploy unreachable → unauthorized + disabled production smoke → F3 Danger Zone UI behind initiation flag → controlled F4 before any public activation. **Do not** enable flags. **Do not** claim deletion is live. **Do not** claim store compliance until end-to-end deletion is proven.
---
...
| APP-041F2 | 10 | WEBSITE | Unreachable initiation route foundation | IMPLEMENTED — UNREACHABLE — PENDING REVIEW | ~2–3 | | APP-041F1 | `POST /api/account/delete`; dual-flag; reauth gate | No UI; both flags off |
...
**APP-041B1**, **APP-041B2a**, **APP-041B2b**, **APP-041B3a**, **APP-041B3b**, **APP-041C1**, **APP-041C2**, **APP-041C3**, **APP-041D0**, **APP-041D1**, **APP-041E1**, **APP-041E2**, **APP-041E3a**, **APP-041E3b**, **APP-041E4a**, **APP-041E4b**, **APP-041E4c**, and **APP-041E4d** are **COMPLETE**. **APP-041F1** is **APPROVED**. **APP-041F2** is **IMPLEMENTED — UNREACHABLE — PENDING REVIEW** (`POST /api/account/delete`; dual-flag gate; no UI; both flags off). APP-041 / APP-041B remain **IN PROGRESS**. Exact next: **review F2 → commit/deploy unreachable → unauthorized+disabled smoke → F3 Danger Zone behind flag → controlled F4 before public activation**. Parallel: APP-065, APP-015, APP-066, APP-021 planning. Do **not** mark APP-041 COMPLETE. Do **not** claim end-to-end deletion works.
