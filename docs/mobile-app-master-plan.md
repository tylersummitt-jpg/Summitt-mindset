...
*Project-control document. Version 1.5.17. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.16 history retained in prior revisions. **v1.5.17 (2026-07-21) — store-submission prep package:** `docs/store-submission/` Apple + Google metadata, privacy/Data Safety drafts, review notes, reviewer plan, screenshot shot list, open items. APP-042 drafting COMPLETE for copy-ready package; portal submit still blocked on D-U-N-S/assets. **v1.5.16:** APP-015 physical PASS. **v1.5.15:** public in-app deletion E2E PASS. **v1.5.14:** discoverability. **v1.5.13:** Create account + membership-gate PASS. **v1.5.12:** APP-021/022 + controlled backend E2E.*
...
| Current phase | **Store / portal path** — auth, membership gate, deletion, APP-015 links PASS; **store metadata/privacy draft package READY** in `docs/store-submission/`. Apple enrollment waiting on **D-U-N-S**. Icon/splash open (Brooke). TestFlight open. Android deferred. |
| Current assigned task IDs | APP-049+ (Apple Developer when D-U-N-S available / App Store Connect), APP-046–048 (icon/splash/screenshots when assets ready), APP-042 portal finalize (Tyler decisions + unresolved Pixel/PrivacyInfo), APP-040, APP-065, APP-066 |
| Last completed task IDs | **APP-042 store-submission draft package (2026-07-21)**; **APP-015 physical PASS**; public in-app deletion E2E PASS; Create account + membership-gate PASS; APP-041 controlled E2E; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash; Tyler store-copy decisions + Meta Pixel / PrivacyInfo unresolved items; TestFlight. Account deletion and APP-015 no longer blockers. APP-065 parallel. Android unstarted. |
...
| Exact next task | **1)** Tyler decisions on subtitle/category/Pixel/privacy-policy gaps (`docs/store-submission/store-submission-open-items.md`). **2)** Complete Apple organization enrollment when **D-U-N-S** arrives (APP-049+). **3)** Add final icon and launch assets when Brooke finishes (APP-046–047); capture screenshots per shot list. **4)** Create App Store Connect record and configure signing. **5)** Archive and upload TestFlight (APP-050). **6)** Android implementation and Play Console afterward. Parallel: **APP-065**, **APP-066**. Do **not** add IAP. Do **not** add Sign in with Apple for V1. |
...
1. **Tyler store decisions + unresolved privacy items** — subtitle/category; production Meta Pixel; privacy-policy provider naming; PrivacyInfo completeness (`docs/store-submission/`).
...
**APP-042 store-submission draft package COMPLETE (2026-07-21)** in `docs/store-submission/`. Exact next: **1)** Tyler decisions (subtitle/category; production Meta Pixel; privacy-policy gaps; PrivacyInfo completeness). **2)** Complete Apple organization enrollment when **D-U-N-S** arrives. **3)** Add final icon and launch assets when Brooke finishes; capture screenshots. **4)** Create App Store Connect record and configure signing. **5)** Archive and upload TestFlight. **6)** Android implementation and Play Console afterward. Parallel: APP-065, APP-066. Do **not** invent “no data collected.” Do **not** claim Apple/Google approval. Do **not** put real reviewer secrets in git.

---

*End of master plan v1.5.17. Maintained per §14. Do not let it become aspirational fiction — update statuses and evidence every session.*
