...
| Current phase | Clerk production session config **180 days** applied (APP-062–064 COMPLETE) → **APP-065** elapsed-time validation IN PROGRESS; remaining Phase-2 formal closures (APP-010, APP-016) + production readiness (APP-021 still open) |
| Current assigned task IDs | APP-065 (IN PROGRESS), APP-010, APP-016, APP-041, APP-015, APP-018–APP-021, APP-069 |
| Last completed task IDs | APP-062, APP-063, APP-064 (also: APP-008 Stage 1 PASS, APP-009, APP-011–APP-014, APP-017, APP-059, APP-070) |
| Current blocker | **APP-065** elapsed-time / long-session proof still open (do **not** claim 180-day persistence proven). **APP-010** / **APP-016** formal acceptance remain open. Account deletion (APP-041) Required for V1. Android unstarted. APP-021 formal architecture decision still open. |
...
| Exact next task | **APP-065** — honest **elapsed-time** long-session validation on iPhone (and later Android): do **not** mark COMPLETE from immediate force-close/reopen alone. Parallel open work: **APP-010**, **APP-016**, **APP-069**, **APP-041**. |
...
- **Not claimed:** multi-week or multi-month / full **180-day elapsed** persistence; Google OAuth support; Android support; account deletion completion; push notifications; full external-link coverage; complete App Store readiness; exhaustive testing of every website page; exhaustive testing of every member-data write; that SMS behavior was exercised or changed; that APP-010 / APP-016 / APP-069 are complete; that Client Trust formal gates fully passed; that APP-065 is complete.
- **Preserved open work:** **APP-065** (elapsed-time long-session validation) is **IN PROGRESS**; **APP-010** and **APP-016** require later formal evidence closure; **APP-069** remains open; account deletion (APP-041) remains a **production requirement**; Android remains **unstarted**; Google OAuth remains outside the intended V1 app-only email posture; external-link handling was **not meaningfully exercised** (APP-015 remains open); broader production-readiness and App Store work remains; SMS behavior and website product logic were **not** changed by the mobile POC. Production Clerk is configured for **180 days** (DEC-022) — configuration is done; elapsed proof is not.
...
5. **Broader controlled physical-device testing** — including APP-015 external-link policy when a natural link surface is available; Client Trust (APP-066) — **not** claimed as already fully passed.
...
- **v1.5 dependency notes:** **APP-062, APP-063, APP-064 COMPLETE** (production Clerk Hobby→Pro; max lifetime **180 days**; inactivity off; no website/mobile code). **APP-065 IN PROGRESS** — exact next session task (elapsed-time only; short-cycle post-change reopen already PASS). **APP-010 and APP-016 remain NOT STARTED**; **APP-069 remains NOT STARTED**; **APP-021 remains OPEN**. Do not drop APP-010 / APP-016 from remaining-work view. **APP-041** remains Required for V1. **APP-015** (external links) remains open. Android APP-018+ deferred until iOS path is sufficiently stable. APP-022 still depends on **APP-021**. APP-053/APP-055 depend on **APP-041** and **APP-065**.
...
- **Parallelizable:** APP-041 (deletion design) alongside APP-065; later formal APP-010 / APP-016 / APP-069 evidence closure alongside; privacy/data-safety drafting (APP-042) alongside; Android POC (APP-018..020) only after iOS is stable enough; icons/splash later.
...
- *Stage 1 status (2026-07-17 / corrected v1.4.1; session config updated 2026-07-18):* **PASS for short-cycle live-shell gates that have COMPLETE tasks** — Victory Room load (APP-009), force-close + reboot persistence (APP-011), Ask Pat (APP-013), Film Room/Vimeo (APP-014), offline/retry, internal navigation; results table recorded (APP-017). Email-code login **worked** and subscribed UI access was observed, but **APP-010 and APP-016 remain NOT STARTED**. Production Clerk max lifetime is now **180 days** (DEC-022 / APP-064); post-change short-cycle reopen PASS; **APP-065 IN PROGRESS** — do not claim 180-day/multi-month persistence proven. **Not claimed:** same-Clerk-user identity formally proven; entitlement metadata verified; APP-069 complete; all Phase-2 authentication acceptance closed; every Client Trust requirement passed; multi-day/multi-month elapsed persistence; external-link coverage (APP-015 still open); Android; account deletion; App Store readiness.
...
### CRITICAL PATH
Master plan → policy verify → **iPhone Stage 1 POC PASS (Candidate A2 leading)** → **Clerk Pro + 180-day max lifetime (APP-062–064 COMPLETE)** → **APP-065 elapsed-time validation (IN PROGRESS)** + later formal **APP-010 / APP-016 / APP-069** closures + **account deletion (APP-041)** → formal APP-021 close → production harden / App Store → Android when iOS stable → launch.

### FIRST GO/NO-GO TEST
**Does the Clerk session persist in an iOS WKWebView after force-close/reopen and land the user in Victory Room without re-login?** (Checkpoint A / APP-011.) **Stage 1: PASS** (including full reboot). Production max lifetime now **180 days** (DEC-022); short-cycle post-change reopen PASS. Remaining: **APP-065 elapsed-time proof**, formal APP-010 / APP-016 closure, account deletion, App Store posture, Android.
...
APP-062–APP-064 are COMPLETE (production Clerk Pro; max lifetime **180 days**; inactivity off; DEC-022). **APP-065 is IN PROGRESS** — next work is honest **elapsed-time** long-session validation (do not mark COMPLETE from short-cycle reopen alone). Parallel open: APP-010, APP-016, APP-069, APP-041. No website/mobile code change is required for the session-lifetime setting itself.
