...
| Current blocker | **APP-065** elapsed-time proof still open. **APP-041** parent still open for native-shell deletion discoverability and **public** initiation activation. **Native purchase suppression + combined `/app/sign-in` (Sign in + Create account) implemented** — physical-device sign-up verification required. Android unstarted. |
...
| V1 login posture | **DECIDED (DEC-018 ACTIVE):** app-only first-party **email verification-code** auth on the **same** Clerk instance. Website keeps Google unchanged. **Website `/app/sign-in` supports explicit Sign in + Create account** (custom `useSignIn` / `useSignUp`; no Google/Apple). New accounts receive **no membership entitlement** and land on **/app/membership**. Native Checkout remains blocked. **Sign in with Apple not required for V1.** **Next:** physical-device verify create-account + membership gate. |
