...
| Current blocker | **APP-065** elapsed-time proof still open. **APP-041** parent still open for native-shell deletion discoverability and **public** initiation activation. **Native iOS purchase suppression implemented on website** (UA `SummittMindsetiOS` → `/app/membership`; Checkout 403) — physical-device verification still required. Android unstarted. |
...
| V1 login posture | **DECIDED (DEC-018 ACTIVE):** app-only first-party **email verification-code** auth on the **same** Clerk instance. Website keeps Google unchanged. **Website `/app/sign-in` implements preferred APP-061 custom `useSignIn` email-code flow** (no Google/Apple on that route). **Sign in with Apple not required for V1.** **APP-010 COMPLETE** on physical iPhone (shared sign-in surface). **Next:** point mobile WKWebView start URL at `/app/sign-in` (mobile-repo slice). |
