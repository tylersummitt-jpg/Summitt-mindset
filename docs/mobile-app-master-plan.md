...
| Current phase | **APP-041** IN PROGRESS (APP-041A COMPLETE; **B1**+**B2a**+**B2b**+**B3a**+**B3b** COMPLETE; **APP-041C1** IMPLEMENTED — PENDING REVIEW in worktree — purge matrix docs only; **no public deletion capability**); **APP-065** IN PROGRESS; APP-021 still open |
| Current assigned task IDs | APP-041C1 review, APP-041C2 (NOT STARTED), APP-065 (IN PROGRESS), APP-041 (IN PROGRESS), APP-015, APP-066, APP-018–APP-021 |
| Last completed task IDs | APP-041B2b, APP-041B3b, APP-041B3a, APP-041B2a, APP-041B1, APP-041A, APP-010, APP-016, APP-069, APP-062, APP-063, APP-064 (also: APP-008 Stage 1 PASS, APP-009, APP-011–APP-014, APP-017, APP-059, APP-070) |
...
| Exact next task | **Review/commit APP-041C1** (purge matrix freeze), then controlled **APP-041C2** purge RPC/CAS foundation. Do **not** jump to public UI. Do **not** claim end-to-end deletion works. Parallel: **APP-065**, **APP-015**, **APP-066**, **APP-021** planning. |
...
- **APP-041C1:** **IMPLEMENTED — PENDING REVIEW** (worktree docs only) — canonical purge/anonymization policy freeze in `docs/account-deletion-purge-matrix.md`. Fixed product decisions (STOP minimum evidence, SMS/coaching DELETE, testimonials/admin notes/shipping rules, Stripe/Clerk/tombstone/challenge/shared/external). **No SQL. No migration. No purge execution. No privacy-copy update yet.**
- **APP-041C2 / APP-041C3:** **NOT STARTED** (C2 = purge RPC + CAS `purge_result` wiring after C1 commit; C3 = app orchestrator).
- **Unlink ≠ STOP:** B2a deletes live `sms_identities` / `sms_audience` binding; it does **not** invent STOP rows. Minimum STOP evidence policy is frozen in the C1 matrix (SID + timestamp + command token + one-way phone hash).
- **Rejected for B2a:** phone hash / HMAC secret / new opt-out evidence table / fake STOP evidence (C1 later introduces one-way phone hash **only** for retained STOP tombstone evidence—not a B2a evidence table).
- **APP-041B (parent orchestrator):** **IN PROGRESS** (B1–B3b + B2b complete; purge execution / Clerk delete / public initiate not started).
- **APP-041C–APP-041F:** C1 pending review; C2–F **NOT STARTED** (F = store evidence).
- Exact next action: **review/commit APP-041C1**, then controlled **APP-041C2**. Do **not** claim end-to-end deletion works. Canonical matrix: `docs/account-deletion-purge-matrix.md`.
...
- Application-data purge RPC / anonymization execution (C2+); Clerk user deletion; completion/reconciliation worker; admin recovery for failed/stuck deletion requests
- Public account-deletion API; user-facing Delete Account UI; reauthentication requirement
- Privacy/data-deletion copy update for STOP/Stripe/testimonial/tombstone/provider retention (required before public UI; does not block private C2)
...
| APP-041C1 | Purge/anonymization policy + dependency freeze (docs only) | **IMPLEMENTED — PENDING REVIEW** | ~1–2 |
| APP-041C2 | Server-only purge RPC + CAS `purge_result` wiring (no public API) | NOT STARTED | ~4–8 |
| APP-041C3 | Application purge orchestrator (no public initiation) | NOT STARTED | ~2–4 |
...
### Exact next action
**Review/commit APP-041C1** (canonical purge matrix), then controlled **APP-041C2** (purge RPC + CAS `purge_result`). **Do not** jump to public UI. **Do not** claim store compliance until end-to-end deletion is proven.
---
...
2. **APP-041** — account deletion parent **IN PROGRESS** (B1–B3b + B2b COMPLETE; **APP-041C1** IMPLEMENTED — PENDING REVIEW). Exact next: **review/commit C1 → C2 purge foundation**. No public deletion capability. Canonical matrix: `docs/account-deletion-purge-matrix.md`.
...
| APP-041C1 | 10 | WEBSITE | Purge/anonymization policy + dependency freeze (docs only) | IMPLEMENTED — PENDING REVIEW | ~1–2 | | APP-041B2b | `docs/account-deletion-purge-matrix.md` | **Docs only.** No SQL/migration/purge execution. C2 blocked until C1 committed |
| APP-041C2 | 10 | WEBSITE | Server-only purge RPC + CAS `purge_result` (no public API) | NOT STARTED | ~4–8 | | APP-041C1 | | Service-role RPC; 20-arg CAS; no Clerk/Stripe customer delete; no public endpoint |
| APP-041C3 | 10 | WEBSITE | Application purge orchestrator (no public initiation) | NOT STARTED | ~2–4 | | APP-041C2 | | CAS transitions only; Clerk-last still later |
...
**APP-041B1**, **APP-041B2a**, **APP-041B2b**, **APP-041B3a**, and **APP-041B3b** are **COMPLETE**. **APP-041C1** is **IMPLEMENTED — PENDING REVIEW** (docs only; `docs/account-deletion-purge-matrix.md`; **no migration**; no purge SQL). **APP-041C2 / C3** **NOT STARTED**. APP-041 / APP-041B remain **IN PROGRESS**. Exact next: **review/commit C1 → controlled C2**. Parallel: APP-065, APP-015, APP-066, APP-021 planning. Do **not** mark APP-041 COMPLETE. Do **not** claim end-to-end deletion works.

---

*End of master plan v1.5.10. Maintained per §14. Do not let it become aspirational fiction — update statuses and evidence every session.*
