...
*Project-control document. Version 1.5.25. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.24 history retained in prior revisions. **v1.5.25 (2026-07-24) — production optional password auth PASS recorded (docs-only):** temporary QA password login PASS on `/app/sign-in`; Clerk password signup OFF / add-password ON; Client Trust OFF (documented tradeoff); lockout + bot protection ON; permanent store reviewer account still NOT created; native iOS/Android reviewer password proofs PENDING. **v1.5.24:** complete store-submission package. **v1.5.23:** canonical native platform detection. **v1.5.22:** physical Vimeo `dnt=1` PASS.*
...
| Current phase | **Store / portal path** — store-submission docs COMPLETE; **production optional password auth PASS** (temporary QA); **permanent reviewer account NOT YET CREATED**; Apple enrollment waiting on **D-U-N-S**; Brooke assets open; Play org open; TestFlight open. |
| Current assigned task IDs | Permanent reviewer provision + entitlement/demo + native password proofs; APP-049+ (Apple when D-U-N-S available); APP-046–048 (assets); APP-042 portal finalize; APP-040; APP-065 |
| Last completed task IDs | **Production password sign-in validation PASS (2026-07-24, docs record)**; optional password sign-in implementation (`c62663c`); complete store-submission package docs; canonical native platform detection; Vimeo `dnt=1` PASS; Meta Pixel PASS; APP-015; deletion E2E; create-account + membership gate; APP-041; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: permanent reviewer account + native iOS/Android password proofs; Brooke icon/splash/feature graphic; Tyler subtitle/category; PrivacyInfo; TestFlight; Google Play org; physical Android father-phone after internal testing. |
...
| V1 login posture | **DEC-018 ACTIVE + password option PASS (2026-07-24):** `/app/sign-in` — email-code Sign in + Create account remain primary for ordinary members; optional **Sign in with password** for existing Clerk users with a password factor. Social absent in app; website Google unchanged. Production Clerk: **password signup OFF**; **add password ON**; email-code enabled. New accounts: no entitlement → `/post-sign-in` → **/app/membership**. Store reviewers: dedicated password account (no OTP) — **permanent reviewer NOT YET CREATED**. Native Checkout blocked. **Sign in with Apple not required for V1.** |
| Session-lifetime standard | **Production Clerk Dashboard configured:** maximum lifetime **ENABLED at 180 days**; inactivity timeout **DISABLED** (DEC-022; APP-062–064 COMPLETE). **Not permanent / not indefinite.** Short-cycle force-close/reopen **PASS**. **180-day elapsed persistence NOT proven** (APP-065 IN PROGRESS). **Client Trust: OFF** (2026-07-24) — removes new-device extra verification for password logins to support reusable store-review credentials; documented tradeoff in `reviewer-test-account-plan.md`. Lockout + bot sign-up protection remain **ON**. |
...
| Exact next task | **1)** Create permanent reviewer account + password; entitlement + demo; browser + iOS + Android password proofs (`reviewer-test-account-plan.md`). **2)** Tyler finalize listing decisions (subtitle / category). **3)** Brooke assets. **4)** Apple enrollment when **D-U-N-S** arrives. **5)** Screenshots / TestFlight / Play internal testing. Parallel: **APP-065**. Do **not** add IAP. Do **not** claim permanent reviewer already exists. Do **not** claim iPad support in V1. |
...
| Film Room / Vimeo | RSC lists videos from Supabase; detail page embeds via `buildVimeoPlayerEmbedUrl` → `player.vimeo.com/video/{id}?dnt=1` | `src/lib/vimeo-player-embed.ts`, `src/app/film-room/page.tsx`, `src/app/film-room/[id]/page.tsx` | Yes | **iOS physical PASS (2026-07-21)** with `dnt=1` (HTTP 200, config `dnt = 1`); Android untested | Low (iOS) |
...
| Analytics | Meta Pixel (marketing routes; **native app UA omitted — iOS physical PASS 2026-07-21**; Android marker recognized on website, shell not built) | `src/components/MetaPixelRoot.tsx`, `src/lib/meta-pixel*.ts`, `src/app/layout.tsx`, `src/lib/native-app/platform.ts` | Yes (browser only) | Add app-level analytics/crash (native) | Low |
...
1. **Tyler store listing decisions + remaining privacy items** — subtitle/category; optional Resend naming; PrivacyInfo completeness (`docs/store-submission/`). Public Meta + Vimeo Privacy Policy disclosures COMPLETE; Vimeo `dnt=1` implementation + physical iPhone playback PASS — not open. Native Meta Pixel PASS — not open. Final portal questionnaire entry remains manual.
...
*End of master plan v1.5.24. Maintained per §14. Do not let it become aspirational fiction — update statuses and evidence every session.*
