...
| Current phase | **APP-041** IN PROGRESS (APP-041A COMPLETE; **APP-041B1** in repo not deployed; **APP-041B2a** in worktree only — local SMS unlink foundation, **not committed/deployed/applied**; **no public deletion capability**); **APP-065** IN PROGRESS; APP-021 still open |
| Current assigned task IDs | APP-041B2a (final approval → commit/push → controlled migration apply), APP-065 (IN PROGRESS), APP-041 (IN PROGRESS), APP-015, APP-066, APP-018–APP-021 |
...
| Current blocker | **APP-065** elapsed-time proof still open. **APP-041** Required for V1 — parent IN PROGRESS; **no user account can currently initiate deletion** (no public delete API; B2a = internal SMS unlink foundation only). APP-015 open. Android unstarted. APP-021 open. |
...
| Exact next task | **APP-041B2a final approval → `git add .` → commit and push → controlled apply of B1 + B2a migrations → non-production/staging validation.** No public deletion endpoint yet. Parallel: **APP-065**, **APP-015**, **APP-066**, **APP-021** planning. |
...
*Control record for APP-041. APP-041B1 is in the repository (not deployed). APP-041B2a is **worktree-only**. Neither enables public account deletion. No private identifiers are stored here.*

### Status
- **APP-041 (parent):** **IN PROGRESS** — Required for V1; **not COMPLETE**.
- **APP-041A:** **COMPLETE** (live-schema + Clerk Dashboard verification).
- **APP-041B1:** **COMPLETE in repository** (migration + repository/tests); **not deployed / not applied** to production. **No endpoint, UI, Stripe cancel, Clerk delete, purge, or cron.**
- **APP-041B2a:** implemented in the **worktree only** (uncommitted): atomic local SMS unlink RPC + orchestrator helper, anti-resurrection guards (audience sync / START / onboarding), coach-job cancellation of all nonterminal statuses, final pre-send Twilio gate, START blocked acknowledgment (no false “rejoined”), retry evidence via `steps.sms_binding_removed` (no phone/PII). **Migration not applied.** **No public deletion capability** — no user account can currently initiate deletion.
- **Unlink ≠ STOP:** B2a deletes live `sms_identities` / `sms_audience` binding; it does **not** invent STOP rows. Real STOP evidence remains in existing `sms_inbound_messages`.
- **Rejected for B2a:** phone hash / HMAC secret / new opt-out evidence table / fake STOP evidence.
- **APP-041B (parent orchestrator):** **IN PROGRESS** (B1 + B2a foundation; Stripe/purge/Clerk delete not started; no public initiate).
- **APP-041C–APP-041F:** **NOT STARTED**.
- Exact next action: **(1)** independent final approval **(2)** `git add .` **(3)** commit and push **(4)** controlled application of B1 + B2a migrations **(5)** non-production/staging validation before any public deletion endpoint **(6)** only then the next APP-041 slice. Do **not** begin B3 immediately after commit. Do **not** mark B2a deployed or complete in production.
...
- **Unresolved final purge semantics** — exact delete vs anonymize vs retain per table is **not** finalized.
...
### Exact next action
**After APP-041B2a final approval:** `git add .` → commit and push → controlled apply of B1 + B2a migrations → non-production/staging validation — **do not** expose `POST /api/account/delete` until APP-041C reauth/confirmation exists. **Do not begin B3 immediately after commit.** **Do not claim account deletion works.** **Do not mark B2a deployed.**
...
2. **APP-041** — account deletion parent **IN PROGRESS** (APP-041A COMPLETE; APP-041B1 in repo not deployed; **APP-041B2a in worktree only**). Exact next after final approval: commit/push → controlled B1+B2a migration apply → staging validation. No public deletion capability. Do not start B3 immediately after commit.
...
| APP-041B | 10 | WEBSITE | Durable deletion state + backend orchestrator (parent) | IN PROGRESS | 8–14 | | APP-041A | APP-041B1 + B2a (worktree) | **Not COMPLETE.** B1 in repo (not deployed); B2a SMS unlink foundation in worktree only. No public initiate until APP-041C |
...
| APP-041B2a | 10 | WEBSITE | Local SMS unlink + anti-resurrection + coach final-send gate (no public API) | IN PROGRESS (worktree only) | ~4–6 of B band | | APP-041B1 | migration `20260718130000_account_deletion_sms_suppress.sql` (unapplied); shared eligibility helper; safety+main send gates | **Not committed/deployed/applied.** Unlink ≠ STOP. Atomic `sms_binding_removed` in RPC. After approval: commit/push → controlled migration apply → staging validation (not B3 yet) |
...
**APP-041B2a** is in the worktree only (not committed/deployed/applied) — local SMS unlink + anti-resurrection + coach final-send gate (including safety short-circuit); **migration not applied**; **unlink ≠ STOP**; no phone hash/HMAC/evidence table; **no public deletion capability**. APP-041 / APP-041B remain **IN PROGRESS**. After final approval: **commit/push → controlled B1+B2a migration apply → staging validation** before any public deletion endpoint; **do not begin B3 immediately after commit**. Parallel: APP-065, APP-015, APP-066, APP-021 planning. Do **not** mark APP-041 or B2a COMPLETE/deployed.
