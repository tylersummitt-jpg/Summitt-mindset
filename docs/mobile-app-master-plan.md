...
*Project-control document. Version 1.5.24. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.23 history retained in prior revisions. **v1.5.24 (2026-07-24) — complete store-submission package (docs-only):** phone-only V1 scope (iPhone / Android phone; iPad deferred); final-draft Apple/Google listing copy; reviewer + demo-content plan; review/access notes; App Privacy + Data Safety worksheets; regression + release-config checklists; five-screen phone storyboard; assets/blocker tracker; stale “Android not built” store-package language corrected via `docs/store-submission/waiting-on-assets.md`. **v1.5.23:** canonical native platform detection. **v1.5.22:** physical Vimeo `dnt=1` PASS.*
...
| Current phase | **Store / portal path** — **store-submission docs package COMPLETE** (`docs/store-submission/`); V1 **iPhone-only / Android phone-focused** (iPad deferred); Apple enrollment waiting on **D-U-N-S**; Brooke assets open; Play org open; physical Android (father phone) after Play internal testing; TestFlight open. |
| Current assigned task IDs | APP-049+ (Apple Developer when D-U-N-S available / App Store Connect), APP-046–048 (icon/splash/screenshots when assets ready), APP-042 portal finalize (Tyler listing decisions / PrivacyInfo), APP-040, APP-065, APP-066 |
| Last completed task IDs | **Complete store-submission package docs (2026-07-24)**; **Canonical native platform detection**; **Physical Vimeo `dnt=1` playback PASS**; Meta/Vimeo privacy disclosures; native Meta Pixel PASS; APP-042 drafts; APP-015 PASS; deletion E2E PASS; Create account + membership-gate PASS; APP-041; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash/feature graphic; Tyler subtitle/category/reviewer inbox; PrivacyInfo review; TestFlight; Google Play org; physical Android father-phone after internal testing. Store **copy/worksheets/checklists** no longer blockers. |
...
| Exact next task | **1)** Tyler finalize store listing decisions (subtitle / category / reviewer inbox). **2)** Brooke assets. **3)** Apple enrollment when **D-U-N-S** arrives. **4)** Screenshots (phone only). **5)** TestFlight / Play internal testing → father-phone. Parallel: **APP-065**, **APP-066**. Do **not** add IAP. Do **not** claim “no data collected.” Do **not** claim iPad support in V1. |
