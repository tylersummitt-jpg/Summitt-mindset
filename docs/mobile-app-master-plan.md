...
| Current assigned task IDs | APP-049+ (Apple Developer when D-U-N-S available / App Store Connect), APP-046–048 (icon/splash/screenshots when assets ready), APP-015 (physical-device link verification remaining), APP-040, APP-042, APP-065, APP-066 |
| Last completed task IDs | Website native-app link/navigation audit + fixes (2026-07-21); **Public in-app deletion E2E PASS**; Create account + CAPTCHA + membership-gate PASS; APP-041 controlled E2E PASS; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash; **physical-device link checklist**; store metadata / TestFlight. Account deletion no longer a store-submission blocker. APP-065 parallel. Android unstarted. |
...
| Exact next task | **1)** Physical-device link checklist (APP-015 verification on iPhone — not marked PASS until Tyler runs it). **2)** Apple Developer organization enrollment when **D-U-N-S** is available (APP-049+). **3)** Store metadata, privacy answers, review credentials. **4)** Icon + launch assets when Brooke finishes (APP-046–047). **5)** App Store Connect / signing / archive / TestFlight (APP-050). **6)** Android later. Parallel: **APP-065**, **APP-066**. Do **not** add IAP. Do **not** add Sign in with Apple for V1. |
...
**U15 — External links / new windows / popups.** Native shell policy already: same-origin + Clerk in WKWebView; external HTTPS → SFSafariViewController; mailto/tel/sms → system. Website audit 2026-07-21 closed native purchase-solicitation gaps and unsafe consent blanks. Physical APP-015 checklist remaining. Hours at risk: 1–3 (device verification).
...
| APP-015 | 2 | POC | Test external-link behavior on iOS | IN PROGRESS | 0.5 | | APP-012 | | **Website audit + native-safe link fixes COMPLETE (2026-07-21).** Physical-device checklist remaining — not PASS until Tyler runs it. Mobile allowlist unchanged. |
