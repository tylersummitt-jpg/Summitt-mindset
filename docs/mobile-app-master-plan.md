...
| Current phase | **APP-041** IN PROGRESS (APP-041A COMPLETE; **B1**+**B2a**+**B2b**+**B3a**+**B3b** COMPLETE; **APP-041C1** COMPLETE/committed; **APP-041C2** IMPLEMENTED — PENDING FINAL REVIEW in worktree — purge RPC/CAS not applied; **no public deletion capability**); **APP-065** IN PROGRESS; APP-021 still open |
| Current assigned task IDs | APP-041C2 final review, APP-041C3 (NOT STARTED), APP-065 (IN PROGRESS), APP-041 (IN PROGRESS), APP-015, APP-066, APP-018–APP-021 |
| Last completed task IDs | APP-041C1, APP-041B2b, APP-041B3b, APP-041B3a, APP-041B2a, APP-041B1, APP-041A, APP-010, APP-016, APP-069, APP-062, APP-063, APP-064 (also: APP-008 Stage 1 PASS, APP-009, APP-011–APP-014, APP-017, APP-059, APP-070) |
...
| Exact next task | **Final review APP-041C2**, then controlled migration apply + fake-user ROLLBACK validation, then **APP-041C3** orchestrator. Do **not** jump to public UI. Do **not** claim end-to-end deletion works. Parallel: **APP-065**, **APP-015**, **APP-066**, **APP-021** planning. |
...
- **APP-041C2:** **IMPLEMENTED — PENDING FINAL REVIEW** (worktree; production-schema aligned) — 20-arg CAS; purge RPC with `sms_opt_out_tombstones` (SID+time+token only; **no phone hash**); testimonials/admin rows DELETE; challenge DELETE by exact `clerk_user_id` only (nullable column + index; legacy email-only out of band, non-blocking); helper `purgeAppDataForDeletion`. **Migrations not applied. No production data touched. No public endpoint/UI.**
- **APP-041C3:** **NOT STARTED** (orchestrator + CAS to `app_data_purged`).
- **Unlink ≠ STOP:** B2a deletes live `sms_identities` / `sms_audience` binding; it does **not** invent STOP rows. Minimum STOP evidence is dedicated tombstone rows (SID + received_at + command token) then inbound source delete.
- **Rejected for C2 V1:** phone hash / in-place inbound anonymize / email-matched challenge delete / treating `approved` as publication consent.
- **APP-041B (parent orchestrator):** **IN PROGRESS** (B1–B3b + B2b complete; purge apply/validation / Clerk delete / public initiate not started).
- **APP-041C–APP-041F:** C1 complete; C2 pending final review; C3–F **NOT STARTED** (F = store evidence).
- Exact next action: **final review APP-041C2** → controlled apply + fake-user ROLLBACK validation → **APP-041C3**. Do **not** claim end-to-end deletion works. Canonical matrix: `docs/account-deletion-purge-matrix.md`.
...
| APP-041C2 | Server-only purge RPC + CAS `purge_result` wiring (no public API) | **IMPLEMENTED — PENDING FINAL REVIEW** | ~4–8 |
...
### Exact next action
**Final review APP-041C2** (purge RPC + 20-arg CAS in worktree), then controlled migration apply + fake-user ROLLBACK validation, then **APP-041C3**. **Do not** jump to public UI. **Do not** claim store compliance until end-to-end deletion is proven.
---
...
2. **APP-041** — account deletion parent **IN PROGRESS** (B1–B3b + B2b + C1 COMPLETE; **APP-041C2** IMPLEMENTED — PENDING FINAL REVIEW). Exact next: **final review C2 → apply/validate → C3**. No public deletion capability. Canonical matrix: `docs/account-deletion-purge-matrix.md`.
...
| APP-041C2 | 10 | WEBSITE | Server-only purge RPC + CAS `purge_result` (no public API) | IMPLEMENTED — PENDING FINAL REVIEW | ~4–8 | | APP-041C1 | migrations `20260719120000_…` + `20260719121000_…` (not applied) | **Worktree only.** No production apply. No public endpoint. No Clerk delete. |
...
**APP-041B1**, **APP-041B2a**, **APP-041B2b**, **APP-041B3a**, **APP-041B3b**, and **APP-041C1** are **COMPLETE**. **APP-041C2** is **IMPLEMENTED — PENDING FINAL REVIEW** (20-arg CAS + purge RPC in worktree; **migrations not applied**; no production data touched). **APP-041C3** **NOT STARTED**. APP-041 / APP-041B remain **IN PROGRESS**. Exact next: **final review C2 → controlled apply + fake-user ROLLBACK → C3**. Parallel: APP-065, APP-015, APP-066, APP-021 planning. Do **not** mark APP-041 COMPLETE. Do **not** claim end-to-end deletion works.
