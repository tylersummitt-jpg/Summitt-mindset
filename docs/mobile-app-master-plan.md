...
*Project-control document. Version 1.5.21. Created 2026-07-17. Read-only-audit basis. v1.1–v1.5.20 history retained in prior revisions. **v1.5.21 (2026-07-21) — Vimeo embed privacy posture:** shared `buildVimeoPlayerEmbedUrl` with `dnt=1` on all user-facing player iframes; `/privacy` §6 names Vimeo and technical/playback sharing; store drafts updated; physical iPhone playback after `dnt=1` still pending. **v1.5.20:** public Privacy Policy Meta disclosure. **v1.5.19:** native Meta Pixel physical PASS. **v1.5.18:** code-side native Pixel suppression. **v1.5.17:** store-submission package. **v1.5.16:** APP-015 physical PASS.*
...
| Current phase | **Store / portal path** — auth, membership gate, deletion, APP-015 PASS; native Meta Pixel physical PASS; **public Privacy Policy Meta + Vimeo disclosures COMPLETE** (`dnt=1` embeds); physical Vimeo playback after `dnt=1` pending; store drafts READY. Apple enrollment waiting on **D-U-N-S**. Icon/splash open (Brooke). TestFlight open. Android deferred. |
...
| Last completed task IDs | **Vimeo embed `dnt=1` + Privacy Policy Vimeo disclosure (2026-07-21)**; **Public Privacy Policy Meta disclosure**; **Native Meta Pixel suppression COMPLETE / physical PASS**; **APP-042 store-submission drafts**; **APP-015 physical PASS**; public in-app deletion E2E PASS; Create account + membership-gate PASS; APP-041; APP-022; APP-021 |
| Current blocker | **Apple Developer organization enrollment waiting on D-U-N-S.** Also open: Brooke icon/splash; Tyler store listing copy; PrivacyInfo review; TestFlight; **physical Vimeo playback verify after `dnt=1`**. Website Meta/Vimeo policy and native Pixel no longer blockers. APP-065 parallel. Android unstarted. |
...
1. **Tyler store listing decisions + remaining privacy items** — subtitle/category; optional Resend naming; PrivacyInfo completeness (`docs/store-submission/`). Public Meta + Vimeo Privacy Policy disclosures COMPLETE (`dnt=1` embeds). Physical Vimeo playback after `dnt=1` pending. Native Meta Pixel PASS — not open.
...
*End of master plan v1.5.21. Maintained per §14. Do not let it become aspirational fiction — update statuses and evidence every session.*
