-- Victory Media account-deletion token barrier for discovery.
-- Narrow change to list_account_deletion_requests_for_reconcile only.
-- Happy-path app_data_purged is eligible only after created_at + 2 hours 5 minutes
-- (signed-upload TTL 2h + deterministic 5m margin). All other statuses and
-- failed_retryable backoff are unchanged.
--
-- After apply, manually run: NOTIFY pgrst, 'reload schema';
-- Do NOT embed NOTIFY here.
--
-- Rollback: re-apply 20260719140000_list_account_deletion_requests_for_reconcile.sql
--   (or DROP and recreate from that migration).
      ...
      CASE
        WHEN r.status = 'failed_retryable' THEN
          ...
          + CASE
            ...
            END
        -- Victory Media signed-upload token barrier (2h + 5m). Happy-path only.
        WHEN r.status = 'app_data_purged' THEN
          r.created_at + INTERVAL '2 hours 5 minutes'
        ELSE r.updated_at
      END
    ) <= v_now
  ORDER BY
