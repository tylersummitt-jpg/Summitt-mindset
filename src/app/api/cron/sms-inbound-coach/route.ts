...
import {
  ...
  inboundTurnUnderstandingContextMetadata,
  runInboundTurnUnderstandingContext,
...
function buildInboundOutcomeClaimEvidence(args: {
  userMessage: string;
  commitment: ActiveV2CommitmentRow;
  ...
  const shortAnswerContext = resolveShortAnswerContextAuthority({
    ...
    latestOutboundBody:
      args.memoryPacket.last_outbound_full_body ?? args.lastOutboundSmsPreview,
    latestOpenQuestion:
      args.latestOpenQuestion ?? args.memoryPacket.latest_open_question ?? null,
    expectedAnswerType: args.memoryPacket.open_question_expected_answer_type,
    expectedReplySemantics: args.expectedReplySemantics,
    openQuestionPending: args.memoryPacket.open_question_pending === true,
  ...
  return {
    ...
    latestOpenQuestion:
      args.latestOpenQuestion ?? args.memoryPacket.latest_open_question ?? null,
    expectedReplySemantics: args.expectedReplySemantics ?? null,
    openQuestionPending: args.memoryPacket.open_question_pending === true,
    shortAnswerContext,
    inboundMeaning: args.inboundTurnUnderstandingCtx?.inboundMeaningForPersist ?? null,
...
async function tryPersistInboundAccountabilityOutcomeBeforeSend(
  ...
  const tuCtx = args.turnUnderstandingContext ?? null;
  const inboundMeaning =
    args.inboundMeaningForPersist ??
    tuCtx?.inboundMeaningForPersist ??
    buildInboundMeaningFacts({
      rawInbound: args.userMessage,
  ...
  const turnUnderstandingReconciled =
    args.turnUnderstandingReconciled ?? tuCtx?.reconciled ?? null;
  ...
  const result = await persistInboundAccountabilityOutcomeEvent({
    ...
    payloadJson: {
      ...
      turn_understanding_applied_to_persist: Boolean(
        turnUnderstandingReconciled && !tuCtx?.skippedReason
      ),
      turn_understanding_persist_skip_reason: turnUnderstandingPersistSkipReasonForTelemetry(
        should,
        tuCtx,
        turnUnderstandingReconciled
      ),
      ...(turnUnderstandingReconciled
        ? {
            server_reconciled_persistence_decision:
              turnUnderstandingReconciled.reconciled_persistence_decision,
          }
        : {}),
    },
    idempotencyKey,
...
function turnUnderstandingPersistSkipReasonForTelemetry(
  should: ShouldPersistInboundAccountabilityOutcomeResult,
  tuCtx: InboundTurnUnderstandingContext | null | undefined,
  turnUnderstandingReconciled: ReconciledTurnUnderstanding | null
): string | null {
  if (should.persist) return null;
  if (tuCtx?.skippedReason) return tuCtx.skippedReason;
  if (!should.persist) return should.skipReason;
  if (!turnUnderstandingReconciled) return "turn_understanding_not_available";
  return null;
}
...
async function processV2NormalInboundOutcome(
  ...
  if (
    ...
    const openQForTurnUnderstanding =
    ...
    if (
      ...
      const turnRoutePriorityEarly = buildInboundMeaningRoutePriorityFromV3BuildArgs({
        ...
        openQuestionFacts:
          inboundRelationshipMemoryPacket.open_question_pending === true &&
          Boolean(openQForTurnUnderstanding)
            ? { pending: true }
            : null,
      });
      const inboundMeaningEarly = buildInboundMeaningFacts({
        rawInbound: userMessage,
        classifierEventType: eventType,
        classifierNormalizedHint: normalizedHint,
        routePriority: turnRoutePriorityEarly,
        openQuestionPending:
          inboundRelationshipMemoryPacket.open_question_pending === true ||
          Boolean(openQForTurnUnderstanding),
        latestOpenQuestion: openQForTurnUnderstanding,
      });
      inboundTurnUnderstandingCtx = await runInboundTurnUnderstandingContext({
        inboundBody: userMessage,
        timezone,
        ...
        latestAnswerAfterOpenQuestion:
          inboundRelationshipMemoryPacket.latest_answer_after_open_question ?? null,
        openQuestionPending:
          inboundRelationshipMemoryPacket.open_question_pending === true ||
          Boolean(openQForTurnUnderstanding),
        expectedReplySemantics:
          typeof northStarPktOpenQuestion.expectedReplySemantics === "string"
            ...
            : null,
        recentThreadExcerpt: minimalLinesEarly.slice(-6).join(" | ").slice(0, 500),
        temporalContract: buildTemporalContractForInbound({
          timezone,
          receivedAt: new Date(),
          inboundMeaning: inboundMeaningEarly,
        }),
        proofCalloutClaimSavedAllowed: false,
        expectedAnswerType: inboundRelationshipMemoryPacket.open_question_expected_answer_type,
    ...
    if (v3Resolution) {
      ...
      let gatedOqBody = openVoicePack.voice.body;
      const outcomeClaimEvidenceOq = buildInboundOutcomeClaimEvidence({
        userMessage,
        commitment,
        effectiveBehavior,
        recentEvents,
      ...
      if (!finalGuardsOq.shouldSend) {
          ...
          lastError: formatInboundV3LaneNoSendLastError(
            {
              shouldSend: false,
              body: "",
              noSendReason: finalGuardsOq.noSendReason,
              replySource: "v3_inbound_relationship_lane",
              turnPurpose: "no_send",
              voiceConfidence: 0,
              usedFacts: [],
        ...
        console.warn("[sms-inbound-coach] open_question_final_body_guard_blocked", {
          message_sid: job.message_sid,
          metadata: finalGuardsOq.tuGuard.metadata,
        });
        return;
      }
      gatedOqBody = finalGuardsOq.body;
  ...
  const brainControlGate =
    ...
    }) && !isInboundTurnUnderstandingContextAuthoritative(inboundTurnUnderstandingCtx);

  if (
    isV2SmsConversationBrainControlEnabled() &&
    isV2SmsConversationBrainAllowedForUser(userId)
  ) {
    if (isInboundTurnUnderstandingContextAuthoritative(inboundTurnUnderstandingCtx)) {
      console.info("[v2-sms-conversation-brain]", {
        reason_code: "brain_gate_skipped_turn_understanding_authoritative",
        commitment_id: commitment.id,
        message_sid: job.message_sid,
        failed_safe: Boolean(
          inboundTurnUnderstandingCtx.reconciled?.turn_understanding_failed_safe_fallback
        ),
      });
    } else if (commitmentChangeIntentLikely) {
      console.info("[v2-sms-conversation-brain]", {
  ...
  if (brainControlGate) {
    ...
    if (conversationBrainControlTurn == null && !isV2SmsConversationBrainLegacyFallbackEnabled()) {
      ...
      const outcomeClaimEvidenceLegacy = buildInboundOutcomeClaimEvidence({
        userMessage,
        commitment,
        effectiveBehavior,
        recentEvents,
      ...
      if (!finalGuardsLegacy.shouldSend) {
          ...
          lastError: formatInboundV3LaneNoSendLastError(
            {
              shouldSend: false,
              body: "",
              noSendReason: finalGuardsLegacy.noSendReason,
              replySource: "v3_inbound_relationship_lane",
              turnPurpose: "no_send",
              voiceConfidence: 0,
              usedFacts: [],
        ...
        console.warn("[sms-inbound-coach] legacy_fallback_final_body_guard_blocked", {
          message_sid: job.message_sid,
  ...
  const northStarPktForV3 = buildInboundNorthStarContextPacket({
    ...
    latestOpenQuestionForContext:
      inboundRelationshipMemoryPacket.latest_open_question ?? null,
    expectedAnswerTypeForContext:
      inboundRelationshipMemoryPacket.open_question_expected_answer_type,
    expectedReplySemanticsForContext: null,
    openQuestionPendingForContext: inboundRelationshipMemoryPacket.open_question_pending === true,
    willPersistUserYes: false,
  });
  ...
  if (
    ...
    let gatedPivot = pivotVoicePack.voice.body;
    const outcomeClaimEvidencePivot = buildInboundOutcomeClaimEvidence({
      userMessage,
      commitment,
      effectiveBehavior,
      recentEvents,
        ...
        lastError: formatInboundV3LaneNoSendLastError(
          {
            shouldSend: false,
            body: "",
            noSendReason: finalGuardsPivot.noSendReason,
            replySource: "v3_inbound_relationship_lane",
            turnPurpose: "no_send",
            voiceConfidence: 0,
            usedFacts: [],
    ...
    gatedPivot = finalGuardsPivot.body;
  ...
  if (
    ...
    if (activeReplyCtx.should_force_clarification_for_ambiguous_short_reply) {
      ...
      let gatedClarify = clarifyVoicePack.voice.body;
      const outcomeClaimEvidenceArc = buildInboundOutcomeClaimEvidence({
        userMessage,
        commitment,
        effectiveBehavior,
        recentEvents,
          ...
          lastError: formatInboundV3LaneNoSendLastError(
            {
              shouldSend: false,
              body: "",
              noSendReason: finalGuardsArc.noSendReason,
              replySource: "v3_inbound_relationship_lane",
              turnPurpose: "no_send",
              voiceConfidence: 0,
              usedFacts: [],
      ...
      gatedClarify = finalGuardsArc.body;
  ...
  if (normalInboundV3OwnershipEligible) {
    ...
    if (
      ...
      inboundTurnUnderstandingCtx = await runInboundTurnUnderstandingContext({
        ...
        classifierEventType: eventType,
        classifierNormalizedHint: normalizedHint,
        interpreterRoutePurpose: mainInboundLaneRoutePurpose ?? "normal_inbound_reply",
        routePriority: turnRoutePriority,
        effectiveAsk: effectiveBehavior,
        behaviorStatement: commitment.behavior_statement ?? "",
        lastCoachOutbound:
          inboundRelationshipMemoryPacket.last_outbound_full_body ??
          northStarPktForV3.latestOutboundBody ??
          lastOutboundSmsPreview,
        ...
        expectedReplySemantics:
          typeof northStarPktForV3.expectedReplySemantics === "string"
            ? northStarPktForV3.expectedReplySemantics
            : null,
        recentThreadExcerpt: mainTranscriptLines.slice(-6).join(" | ").slice(0, 500),
        temporalContract: buildTemporalContractForInbound({
          timezone,
          receivedAt: new Date(),
        ...
        proofCalloutClaimSavedAllowed: proofCalloutHint?.proof_callout_claim_saved_allowed === true,
        expectedAnswerType: inboundRelationshipMemoryPacket.open_question_expected_answer_type,
        recentEventsNewestFirst: recentEvents,
        commitmentTitle: commitment.title,
      });
  ...
  const northStarInboundContextPacket = buildInboundNorthStarContextPacket({
    ...
    inboundMeaning: inboundTurnUnderstandingCtx.inboundMeaningForPersist,
    latestOpenQuestionForContext:
      inboundRelationshipMemoryPacket.latest_open_question ??
      northStarPktForV3.latestOpenQuestion ??
      null,
    expectedAnswerTypeForContext:
      inboundRelationshipMemoryPacket.open_question_expected_answer_type,
    expectedReplySemanticsForContext:
      typeof northStarPktForV3.expectedReplySemantics === "string"
        ? northStarPktForV3.expectedReplySemantics
        : null,
    openQuestionPendingForContext: inboundRelationshipMemoryPacket.open_question_pending === true,
  ...
  if (!finalGuardsMain.shouldSend) {
      ...
      lastError: formatInboundV3LaneNoSendLastError(
        {
          shouldSend: false,
          body: "",
          noSendReason: finalGuardsMain.noSendReason,
          replySource: "v3_inbound_relationship_lane",
          turnPurpose: "no_send",
          voiceConfidence: 0,
          usedFacts: [],
        ...
        {
          route_purpose: inboundThreadRoutePurpose,
          branch_name: isLikelyCommitmentChangeIntentTurn(userMessage)
            ? "commitment_change_context_heuristic"
            : null,
        }
      ),
    ...
    console.warn("[sms-inbound-coach] main_inbound_final_body_guard_blocked", {
      message_sid: job.message_sid,
      metadata: guardMeta,
    });
    return;
  ...
  const mainInboundTurnTelemetryArgs = {
    ...
    messageSid: job.message_sid,
    rawBody: userMessage,
    replyBody: finalReplyBody,
    shortAnswerContext: outcomeClaimEvidenceMain.shortAnswerContext ?? null,
    inboundMeaning: inboundTurnUnderstandingCtx.inboundMeaningForPersist,
    turnUnderstandingReconciled: inboundTurnUnderstandingCtx.reconciled,
