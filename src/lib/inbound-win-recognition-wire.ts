/**
 * Inbound Win-recognition orchestration helpers (Umbrella 1).
 * Keeps route.ts thinner while preserving OpenAI as semantic authority.
 */
...
export async function maybePersistInboundWinRecognitionBundle(args: {
  ...
  if (!args.bundle?.result.has_win) return null;
  try {
    return await persistInboundRecognizedWinsBeforeSend({
      ...
      recognition: args.bundle.result,
      fallbackOccurredAtIso: args.fallbackOccurredAtIso ?? null,
    });
  } catch (winPersistErr) {
