...
export function mapTurnUnderstandingToInboundRouteContract(
  ...
  if (tuAuthoritative && confidence >= LOW_CONFIDENCE_THRESHOLD) {
    if (isCoachingFitFeedbackReconciled(reconciled)) {
      return buildPhase1RouteContract({
        route: "legacy_other",
        ...
        forbidden_moves: [
          ...commonForbidden,
          "Repair coaching fit before normal accountability.",
          "Acknowledge the coaching miss — do not defend the system.",
          "Do not continue the same assignment as if nothing happened.",
          "Ask one useful recalibration question (what is off or whether the goal is still right).",
          "Do not ask what got in the way or run outcome triad on the standard.",
        ],
        outcome_to_persist: "none",
      });
    }
...
export function buildTurnUnderstandingUserPrompt(args: BuildTurnUnderstandingPromptArgs): string {
  const lines: string[] = [
    ...
    "- If the conversational meaning itself is genuinely unclear, confidence < 0.55 and response_intent unclear_clarify.",
    "- Do NOT use relationship_meaning unclear merely because accountability / Current Goal completion is unresolved. A message may clearly answer a reflection, preference, or personal question and still require commitment_outcome_recommendation no_outcome_write.",
    "- If the user clearly answered the pending open question, do not choose unclear_clarify just to force them to restate it because it does not fit a goal-status bucket.",
    ...
    "",
    "OPEN-REFLECTION / PRIDE / PERSONAL ANSWER (when expected_reply_semantics is open_reflection, or the latest open question is an open reflection / pride / gratitude / meaning ask):",
    "- The user may be directly answering that open question. Judge whether the inbound actually answers it — do not auto-label every inbound as understood.",
    "- A valid answer may describe an accomplishment, pride, meaningful family time, recovery, returning to work, consistency, faith, mindfulness, a personal realization, or coaching/relationship preference — polished grammar is not required.",
    "- The answer does NOT need to mention the Current Goal, use yes/no language, prove accountability completion, or request a goal change.",
    "- When the inbound concretely answers the open question: do NOT use relationship_meaning unclear merely because it is outside the Current Goal. Prefer an existing accurate meaning: emotional_reflection, direct_answer, or prior_ask_satisfied (use support_request when they ask for support).",
    "- Set last_ask_satisfied yes and answered_last_coach_ask yes when supported; prefer response_intent acknowledge_prior_ask_satisfied or close_loop_no_new_action; set commitment_outcome_recommendation no_outcome_write and persistence_safety do_not_write_but_acknowledge or defer_to_server unless separate clear accountability evidence for the Current Goal is present.",
    "- Do NOT invent Win/Victory Room/proof-saved claims. Do NOT set write_user_yes / write_user_no / write_user_partial from a reflection answer alone.",
    "- Brief pattern answers (e.g. \"Up early every morning\") can still answer a pride/reflection ask as emotional_reflection or direct_answer — not unclear solely because short, and not automatic Current Goal completion.",
    "- Abbreviated English in context of a pride ask (e.g. \"Over my illness and going back to work\") can mean recovery / returning to work as the proud thing — understand without fabricating extra facts.",
    "- Positive coaching preference (e.g. keep motivational texts coming) or positive feedback that the coaching helps (e.g. keeps me mindful): prefer support_request or direct_answer (or emotional_reflection when sharing how it feels). Use coaching_fit_feedback only for fit/relevance complaints — not for positive preference. no_outcome_write; not unclear solely because it is not a goal-status update.",
    "- Truly ambiguous, contradictory, nonsensical, empty, or nonresponsive messages may remain unclear / unclear_clarify.",
    "- If the same message also independently proves Current Goal completion or a miss, normal accountability meaning rules may still apply for that evidence — do not erase clear reflection meaning, and do not invent outcomes.",
    "",
...
/** True when reconciled TU semantically flags coaching fit / relevance repair (not goal-change). */
export function isCoachingFitFeedbackReconciled(
  tu: ReconciledTurnUnderstanding | null | undefined
): boolean {
  if (!tu) return false;
  ...
  return (
    tu.reconciled_relationship_meaning === "coaching_fit_feedback" ||
    tu.reconciled_response_intent === "repair_coaching_fit"
  );
}

export function isCoachingFitFeedbackRelationshipMeaning(
  meaning: string | null | undefined
): boolean {
  return meaning?.trim() === "coaching_fit_feedback";
}

export function isCoachingFitFeedbackResponseIntent(
