...
describe("buildRelationshipPacketForOpenAI snapshot v2 integration", () => {
  ...
  it("always emits proof_and_praise_permission on all surfaces", () => {
    ...
    expect(guided.snapshot.no_send_and_silence_history.authority).toBe("structured_recent_truth");
  });

  it("always emits no_send_and_silence_history on all surfaces", () => {
    const pending = buildActivePendingStateFromCommitmentRow(null);
    const basePacket = buildRelationshipPacketForOpenAI({
      lane: "inbound",
      sourceFacts: {
    ...
    for (const surface of ["inbound", "daily", "weekly", "guided_contract"] as const) {
      ...
      expect(snapshot.no_send_and_silence_history.data.writer_guidance.do_not_explain_internal_message_failure).toBe(
        true
      );
  ...
  it("prompt guidance covers proof permission and final guard separation", () => {
    ...
    expect(guidance).toContain("no_send_and_silence_history");
