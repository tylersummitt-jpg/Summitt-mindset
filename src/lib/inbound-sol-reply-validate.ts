...
function claimsVictoryRoomPersistence(body: string): boolean {
  const t = body.toLowerCase();
  return /\bvictory\s*room\b/.test(t) && /\b(saved|logged|added)\b/.test(t);
}
...
export function evaluateInboundSolBlockOnlyReply(args: {
  ...
  if (args.lifeWinFreshlyInserted !== true && claimsVictoryRoomPersistence(body)) {
