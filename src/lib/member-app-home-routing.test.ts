...
describe("member app home routing (Package 5)", () => {
  ...
  it("Victory Room surfaces Update my goal with dashboard-aligned guards", () => {
    ...
    expect(topCard).toContain("vrAccentLink");
    expect(topCard).toContain("min-h-11");
    expect(topCard).not.toContain("vrFoundationBtn");
  ...
  it("Victory Room does not surface text-check-ins connection notice", () => {
    ...
    expect(page).toContain("Victory Room");
  ...
  it("Victory Room top card restores subheader without legacy hero or foundation blocks", () => {
    const page = readSrc("src/app/dashboard/victory-room/page.tsx");
    const topCard = readSrc("src/components/VictoryRoomTopCard.tsx");
    ...
    expect(page).toContain("VictoryRoomTopCard");
    expect(page).toContain("Victory Room");
    expect(page).not.toContain("A place to remember who you&apos;re becoming");
    expect(page).not.toContain("saved from your");
    expect(page).not.toContain("real choices.");
    expect(topCard).not.toContain("Victory Room");
    expect(topCard).not.toContain("A place to remember who you&apos;re becoming");
    expect(topCard).not.toContain("calm home for proof");
