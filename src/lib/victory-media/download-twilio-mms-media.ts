...
export class TwilioMmsDownloadError extends Error {
  ...
  constructor(
    ...
    opts?: { httpStatus?: number | null; retryable?: boolean; cause?: unknown }
  ) {
    super(code);
...
function mapAbortOrNetwork(e: unknown): never {
  if (e instanceof TwilioMmsDownloadError) throw e;
  if (e instanceof ContentLengthTooLargeError) {
    throw new TwilioMmsDownloadError("content_length_too_large", { retryable: false });
  }
  if (e instanceof StreamByteLimitExceededError) {
    throw new TwilioMmsDownloadError("byte_limit_exceeded", { retryable: false });
  }
  const name = e instanceof Error ? e.name : "";
  const msg = e instanceof Error ? e.message : String(e);
  if (name === "AbortError" || /aborted|timeout/i.test(msg)) {
    throw new TwilioMmsDownloadError("timeout", { retryable: true, cause: e });
  }
  throw new TwilioMmsDownloadError("network_error", { retryable: true, cause: e });
}
...
export async function downloadTwilioMmsMediaBytes(
  ...
  if (firstRes.status >= 200 && firstRes.status < 300) {
    try {
      ...
      if (bytes.length === 0) {
        throw new TwilioMmsDownloadError("empty_body", { retryable: true });
      }
      return {
        bytes,
  ...
  if (firstRes.status === 301 || firstRes.status === 302 || firstRes.status === 303 || firstRes.status === 307 || firstRes.status === 308) {
    const location = firstRes.headers.get("location");
    if (!location) {
      throw new TwilioMmsDownloadError("invalid_redirect", { retryable: true });
    }
    let cdnUrl: URL;
    try {
        ...
        msg === "redirect_host_forbidden"
          ...
          : msg === "redirect_not_https"
            ...
            : msg === "redirect_userinfo_forbidden"
              ? "redirect_userinfo_forbidden"
              : "invalid_redirect";
      throw new TwilioMmsDownloadError(code, { retryable: false });
    }
    ...
    if (secondRes.status >= 300 && secondRes.status < 400) {
      throw new TwilioMmsDownloadError("second_redirect_forbidden", { retryable: false });
    }
...
export async function listTwilioMessageMediaSids(
  ...
  if (res.status < 200 || res.status >= 300) {
    ...
    throw new TwilioMmsDownloadError(code, { httpStatus: res.status, retryable });
  }
