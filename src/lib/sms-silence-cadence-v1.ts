...
export const SILENCE_CADENCE_ROUTE_CARDS: Record<SilenceCadenceRoute, SilenceCadenceRouteCard> = {
  ...
  recommit_or_adjust_day7: {
    ...
    tone: "firm_fair",
    writer_purpose: "recommit or change the goal",
    max_questions: 1,
    allow_goal_adjustment_language: true,
    current_standard_reference: "yes",
    must_do: ["ask whether to recommit or adjust the goal"],
    must_not_do: [...GLOBAL_MUST_NOT_DO, ...BEFORE_DAY_14_MUST_NOT],
  ...
  relationship_check_day10: {
    ...
    writer_purpose: "not annoying but user must participate",
    max_questions: 1,
    allow_goal_adjustment_language: false,
    current_standard_reference: "light",
    must_do: [
      "say you are not trying to be annoying",
      "ask if they want to get back on track",
    ],
    must_not_do: [
      ...GLOBAL_MUST_NOT_DO,
      ...BEFORE_DAY_14_MUST_NOT,
  ...
  honest_decision_day12: {
    ...
    allow_goal_adjustment_language: true,
    current_standard_reference: "light",
    must_do: ["ask honestly if the goal still matters right now"],
    must_not_do: [...GLOBAL_MUST_NOT_DO, ...BEFORE_DAY_14_MUST_NOT],
