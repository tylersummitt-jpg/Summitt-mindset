...
type RescueTokenPayload = {
  clerk_user_id: string;
  exp: number; // unix seconds
};

function getSecret(): string {
  const s = process.env.RESCUE_TOKEN_SECRET;
  if (!s) throw new Error("Missing RESCUE_TOKEN_SECRET");
  return s;
}
...
export function createRescueToken(args: {
  clerk_user_id: string;
  ttlDays?: number;
}): string {
  ...
  const payload: RescueTokenPayload = {
...
export function verifyRescueToken(token: string):
  ...
  try {
    ...
    const payload = JSON.parse(payloadJson) as RescueTokenPayload;
