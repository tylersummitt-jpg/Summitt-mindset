...
**Weekly Pat Pause notebook health (post-noon):** After Sunday noon sends, run [`supabase/manual/sms_weekly_notebook_health_check.sql`](../../supabase/manual/sms_weekly_notebook_health_check.sql). Pass when `weekly_memory_packet_used=true`, `weekly_thread_primary_fetch_succeeded=true`, `weekly_thread_fetch_error_count=0`, `weekly_thread_schema_fallback_used=false`, `weekly_thread_source_candidate_count>0`, `weekly_thread_message_count>1`, and `weekly_thread_fallback_used=false` for known-history users. Fail when `weekly_memory_packet_build_failed=true`, fallback-only thread, or fetch errors without recovered source rows.
