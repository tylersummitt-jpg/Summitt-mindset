# SMS Daily Command Center — SQL Guide (v2.15)

**Weekly Pat Pause notebook health (post-noon):** After Sunday noon sends, run [`supabase/manual/sms_weekly_notebook_health_check.sql`](../../supabase/manual/sms_weekly_notebook_health_check.sql). Pass when `weekly_thread_correct_notebook_verified=true`, `weekly_thread_notebook_failure_reason=none`, `weekly_memory_packet_used=true`, `weekly_thread_primary_fetch_succeeded=true`, `weekly_thread_fetch_error_count=0`, `weekly_thread_schema_fallback_used=false`, `weekly_thread_source_candidate_count>0`, `weekly_thread_exact_source_message_count>0`, `weekly_thread_message_count>1`, and `weekly_thread_fallback_used=false` for known-history users. Fail on any named `weekly_thread_notebook_failure_reason` or `weekly_notebook_health` other than `correct_notebook_verified`. `unclassified_notebook_failure` is a P1 telemetry bug.

**Daily notebook health (Q02):** Per-row `daily_notebook_health` on coach daily outbound rows. Pass when `daily_thread_correct_notebook_verified=true` and `daily_thread_notebook_failure_reason=none`. Fail on any other named reason; `unclassified_notebook_failure` is a P1 telemetry bug (not `needs_review`).

v2.15 adds **daily notebook verification parity**: `daily_thread_correct_notebook_verified`, `daily_thread_notebook_failure_reason`, `daily_thread_message_source_breakdown`, `daily_thread_exact_source_message_count`, `daily_thread_last_outbound_fallback_message_count`, and `daily_notebook_health` on Q02.
